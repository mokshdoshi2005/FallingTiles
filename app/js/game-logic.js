...
Start_Button.addEventListener("click", e => {
    console.log("STARTED")
    console.log(Start_Button.parentElement)
...
function reappearingBlock(listOfCells,fallen__x,fallen__y,Fallen_Ary) {
    for (let loop = 0;loop < listOfCells.length;loop++) {
        if (listOfCells[loop].classList.contains('fallen') && listOfCells[loop].dataset.x==fallen__x && listOfCells[loop].dataset.y==fallen__y) {
            listOfCells[loop].classList.remove('fallen')
            listOfCells[loop].style.animation = 'reappear 2s ease 500ms forwards'
            // updateAry = [fallen__x,fallen__y]
            // UpdateFallen_Ary(updateAry,Fallen_Ary)
            console.log('reappeared',listOfCells[loop])
        }
    }
}
...
export function reappearingBlock2 () {
